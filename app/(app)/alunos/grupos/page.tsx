...
import { listConditionOptions } from "@/features/exercises/queries";
...
export default async function GroupsPage() {
  ...
  const [groups, conditions] = await Promise.all([listGroups(), listConditionOptions()]);
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8 lg:py-6">
      ...
      <GroupsManager groups={groups} canDelete={session.role === "owner"} conditions={conditions} />
    </div>
  );
}
