...
para o owner com consentimento só declarado; trava `organizations.is_seed` no `seed --reset`. **2.8.1 (plano) e 2.8.2 (exercício no plano) ✔. Próxima: 2.8.3** (prescrição da série). Nada em andamento no código (working tree limpo, `tsc` sem erros).
...
3. **2.8** Ajustes do montador — plano aprovado (`~/.claude/plans/…` e `docs/planos/etapa-2.8.md`). 2.8.0 banco ✔
   (migrations `20261004120000`–`20261005120100`, 253 testes no total). 2.8.1 plano ✔, 2.8.2 exercício no plano ✔. Faltam: 2.8.3 série (depois dela, remover as colunas legadas), 2.8.4 produtividade.
...
## Próximo comando
**"Siga com a 2.8.3"** — plano em `docs/planos/etapa-2.8.md`; parar ao final da 2.8 para teste. Antes, confira o estado com:
```bash
